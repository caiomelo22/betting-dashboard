...
module.exports = {
    async up(queryInterface, Sequelize) {
        await queryInterface.bulkInsert('leagues', [
            { id: 61, name: 'Ligue 1 (France)', sportId: 1, createdAt: new Date(), updatedAt: new Date() },
        ]);
        const { readFile } = require('fs/promises');
        let teams = JSON.parse(await readFile(`src/utils/teams/france_ligue_1.json`, "utf8"));
        let teamsToInsert = [];
        ...
        for (let i = 0; i < teams.length; i++) {
            ...
            leagueTeamsToInsert.push({ teamId: teams[i].team.id, leagueId: 61, season: 2022, createdAt: new Date(), updatedAt: new Date() });
        }

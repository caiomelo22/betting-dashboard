        ...
        <v-form ref="form" lazy-validation @submit.stop.prevent="submit">
            <v-card-text>
                        ...
                        <tbody>
                            <tr v-for="(b, i) in parlay.bets" :key="i">
                                ...
                                <td>
                                    ...
                                    <v-icon @click="remove_bet_click(i)" color="red" class="ml-2">mdi-trash-can</v-icon>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <v-form ref="betForm" lazy-validation @submit.stop.prevent="save_bet_click">
                    <v-row align="center" v-if="bet" class="mt-6">
                        <v-col cols="12" md="6">
                            <span class="text-field-label">League</span>
                            <v-select v-model="bet.leagueId" :items="leagues" item-text="name" item-value="id" outlined
                        ...
                        <v-col cols="12" md="6">
                            ...
                            <v-select v-model="bet.homeTeam" @change="bet.homeTeamId = bet.homeTeam.id"
                                :disabled="!bet.leagueId" :items="teamOptions.filter((x) => x.id != bet.awayTeamId)"
                                item-text="name" return-object outlined dense
                                :rules="[validationService.required(bet.homeTeamId)]" class="text-field" />
                        </v-col>
                        <v-col cols="12" md="6">
                            <span class="text-field-label">Away Team</span>
                            <v-select v-model="bet.awayTeam" @change="bet.awayTeamId = bet.awayTeam.id"
                                :disabled="!bet.leagueId" :items="teamOptions.filter((x) => x.id != bet.homeTeamId)"
                                item-text="name" return-object outlined dense
                                :rules="[validationService.required(bet.awayTeamId)]" class="text-field" />
                        </v-col>
                        <v-col cols="12" md="6">
                        ...
                        <v-col cols="12" md="6" v-else-if="bet.type == 'Total'">
                            <div>
                                <NumberField field-title="Line" :field-value="bet.line" :type="numberFieldEnum.Line"
